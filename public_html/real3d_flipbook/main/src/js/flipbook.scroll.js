...
FLIPBOOK.BookScroll = function (el, wrapper, main, options) {
    ...
    this.iscroll = new IScroll(this.wrapper, {
        ...
        disablePointer: !this.options.isMobile,
    });
...
FLIPBOOK.BookScroll.prototype.updateTool = function () {
    const spans = document.querySelectorAll('.flipbook-textLayer span');

    switch (this.options.main.tool) {
        case 'toolSelect':
            for (let element of spans) {
                element.style.pointerEvents = 'auto';
                element.classList.remove('flipbook-noselect');
            }
            this.iscroll.options.disablePointer = false;
...
FLIPBOOK.BookScroll.prototype.onResize = function () {
    ...
    // next three lines will force the iscroll resize and fix the scroller x position after resize from scrollbar to no scrollbar
    if (this.zoom) {
        this.iscroll.refresh();
...
FLIPBOOK.BookScroll.prototype.zoomTo = function (zoom, time, x, y) {
    ...
    this.iscroll.zoom((zoom * this.main.wrapperW) / this.pageWidth, 0, 0, 0);
...
FLIPBOOK.PageScroll = function (book, bookWrapper, main, options, index, texture, html) {
    ...
    const observer = new IntersectionObserver(
        ...
        { root: bookWrapper, threshold: [0, 0.1, 0.5] }
    );
