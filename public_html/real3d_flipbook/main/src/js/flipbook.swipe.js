...
FLIPBOOK.BookSwipe = function (el, wrapper, main, options) {
    ...
    options.pages.forEach((page, index) => {
        const newPage = new FLIPBOOK.PageSwipe(main, options, index, page.src, page.htmlContent);
...
FLIPBOOK.BookSwipe.prototype.onSwipe = function (event, phase, direction) {
    if (phase == 'start') {
        return;
    }
...
FLIPBOOK.PageSwipe = function (main, options, index, texture, html) {
    ...
    if (options.pagePreloader) {
        this.$preloader = jQuery(
            '<img src="' + options.pagePreloader + '" class="flipbook-page-preloader-image">'
        ).appendTo(this.$inner);
    } else {
        this.$preloader = jQuery('<img src="' + options.assets.spinner + '" class="flipbook-page-preloader">').appendTo(
            this.$inner
        );
    }
