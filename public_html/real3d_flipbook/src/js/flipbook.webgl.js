...

{
    ...
    FLIPBOOK.BookWebGL.prototype.onSwipe = function (event, phase, direction, distance, duration, fingerCount) {
        ...
        if ((phase == 'cancel' || phase == 'end') && fingerCount <= 1) {
            ...
            if (distance < 0 && (!right || !right.dragging)) {
                ...
                this.nextPage();
            } else if (distance == 0 && this.clickedPage) {
                this.clickedPage.isFlippedLeft ? this.prevPage() : this.nextPage();
            } else {
                if (left) {
                    left._setAngle(180);
                    left.flipFinished();
                    this.updateHtmlLayer(true);
                }
                if (right) {
                    right._setAngle(0);
                    right.flipFinished();
            ...
            if ((!right || !right.dragging) && (!left || !left.dragging)) {
                if (distance != 0) {
                    if (direction == 'right' && left && (!right || !right.dragging) && this.prevEnabled) {
                        left._setAngle(distance);
                        if (prev) {
                            prev.showMat();
                        }
                    } else if (direction == 'left' && right && this.nextEnabled) {
                        right._setAngle(distance);
                        if (next) {
                            next.showMat();
                        }
                    }
    ...
    FLIPBOOK.BookWebGL.prototype._move = function (e) {
        ...
        var point = e;
        var deltaX = (point.pageX - this.pointX) * 0.5;
        var deltaY = (point.pageY - this.pointY) * 0.5;
        ...
        if (!this.mouseDown) {
            ...
            if (this.options.rotateCameraOnMouseMove) {
                this.tilt =
                    this.options.tiltMin2 +
                    (this.options.tiltMax2 - this.options.tiltMin2) * (1 - this.pointY / jQuery(this.canvas).height());
                this.pan =
                    this.options.panMin2 +
                    ((this.options.panMax2 - this.options.panMin2) * this.pointX) / jQuery(this.canvas).width();
                this.updateCameraPosition();
            }
        ...
        if (this.onMouseMove == 'scroll') {
            ...
            var left = this.getLeftPage();
            if (
                !this.options.rotateCameraOnMouseMove &&
                this.options.rotateCameraOnMouseDrag &&
                (!left || !left.dragging) &&
                (!right || !right.dragging)
            ) {
