...
class Authentication extends ClientsController
{
    ...
    public function login()
    {
        ...
        if($isAjax) {
            $resp["status"] = 0;
            $resp["message"] = "Please enter Username and Password";
            die(json_encode($resp));
        }
    ...
    public function register()
    {
        ...
        if (get_option('allow_registration') != 1 || is_client_logged_in()) {
            if($isAjax == true) {
                return json_encode(['status'=>1, 'loggedin'=>1, 'message'=>'Client successfully logged in']);
                die();
            }
            redirect(site_url());
        }
            ...
            if ($this->form_validation->run() !== false) {
                ...
                if ($clientid) {
                    ...
                    if (get_option('customers_register_require_confirmation') == '1') {
                        ...
                        if($isAjax == true) {
                            return json_encode(['status'=>1, 'confirmation'=>1, 'loggedin'=>0, 'message'=>_l('customer_register_account_confirmation_approval_notice')]);
                            die();
                        }
                    ...
                    if ($logged_in) {
                        ...
                        if($isAjax == true) {
                            return json_encode(['status'=>1, 'loggedin'=>0, 'message'=>_l('clients_account_created_but_not_logged_in')]);
                            die();
                        }
                    ...
                    send_customer_registered_email_to_administrators($clientid);
                    if($isAjax == true) {
                        return json_encode(['status'=>1, 'loggedin'=>1, 'message'=>_l('clients_successfully_registered')]);
                        die();
                    }
                    redirect($redUrl);
                }
        ...
        if ($this->input->post()) {
            if ($this->form_validation->run() !== false) {
                ...
                if (is_array($success) && isset($success['memberinactive'])) {
                    ...
                    $resp["status"] = 1;
                    $resp["type"] = 'danger';
                    $resp["message"] = _l('inactive_account');
                } elseif ($success == true) {
                    set_alert('success', _l('check_email_for_resetting_password'));
                    $resp["status"] = 1;
                    $resp["type"] = 'success';
                    $resp["message"] = _l('check_email_for_resetting_password');
                } else {
                    set_alert('danger', _l('error_setting_new_password_key'));
                    $resp["status"] = 1;
                    $resp["type"] = 'danger';
                    $resp["message"] = _l('error_setting_new_password_key');
                }
    ...
    public function reset_password($staff, $userid, $new_pass_key)
    {
        ...
        if (!$this->Authentication_model->can_reset_password($staff, $userid, $new_pass_key)) {
            ...
            $resp["status"] = 1;
            $resp["type"] = 'danger';
            $resp["message"] = _l('password_reset_key_expired');
        ...
        if ($this->input->post()) {
            if ($this->form_validation->run() !== false) {
                ...
                if (is_array($success) && $success['expired'] == true) {
                    ...
                    $resp["status"] = 1;
                    $resp["type"] = 'success';
                    $resp["message"] = _l('password_reset_message');
                } else {
                    set_alert('danger', _l('password_reset_message_fail'));
                    $resp["status"] = 1;
                    $resp["type"] = 'danger';
                    $resp["message"] = _l('password_reset_message_fail');
                }
