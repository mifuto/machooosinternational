 ...
 */
    ...
    public function password($data, $type)
    {
        ...
        if ($type == 'forgot') {
            ...
            if($isAjax) {
                $fields['{reset_password_url}'] = site_url('../?reset_password=1&flag=0&uid=' . $data['userid'] . '&token=' . $data['new_pass_key']);
            }
        } elseif ($type == 'set') {
            $fields['{set_password_url}'] = site_url('authentication/set_password/0/' . $data['userid'] . '/' . $data['new_pass_key']);
            if($isAjax) {
                $fields['{set_password_url}'] = site_url('../?set_password=1&flag=0&uid=' . $data['userid'] . '&token=' . $data['new_pass_key']);
            }
        }
