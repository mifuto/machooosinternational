...
function getuSignatureAlbums(id) {
    ...
    // formData.append('method', 'getSignatureAlbums');
      ...
      console.log(users);
        if(users != ""){
            ...
            $.each(users, function(key,value) {
                ...
                tabsTitle += '<li class="nav-item" role="presentation"><button class="nav-link '+active+'" id="'+value.folder_name+'-tab" data-bs-toggle="tab" data-bs-target="#'+value.folder_name+'" type="button" role="tab" aria-controls="'+value.folder_name+'" aria-selected="true" onclick="getAlbumFiles(\''+jjj+'\', '+value.user_id+', '+ value.id +')">'+value.folder_name+'</button></li>';
...
function getAlbumFiles (folder, userId, albumId){
   
    // var file_folder = userId+"_"+folder;
    var img_folder = userId+"_"+folder;
    console.log("LogedUser id =======", img_folder);
    var file_folder = folder;
    var data = { "function": 'SignatureAlbum',"method": "getFilesFromFolder", "folderName": img_folder, "albumId": albumId};
        $.ajax({
            ...
            success: function(result){
                ...
                // tabContents += '<ul class="rem-masonry my-masonry">';
                // var tabContents = '';
                // tabContents += '<div class="masonry">';
                // tabContents += '<div class="sb-widget-wrap fl-wrap">';
                // tabContents += '<div class="sb-widget  fl-wrap">';
                // tabContents += '<div class="categories_widget-wrap">';
                ...
                $.each(images, function(key1,value1) {
                    ...
                    var thumb_img = "admin/signatureAlbumUploads/"+img_folder+"/thumbnails/"+file_name;
                    var img = "admin/signatureAlbumUploads/"+img_folder+"/"+file_name;
                    ...
                    // tabContents += '<div class="grid-item-holder_title">';
                    // tabContents += '<h5>Outdoor</h5>';
                    // tabContents += '<h3><a href="portfolio-single.html" class="ajax">Alone on Nature</a></h3>';
                    // tabContents += '</div>';
                    tabContents += '</div>';
                    tabContents += '</div>';
                    // tabContents += '</div>';
                    // tabContents += '<li><img src="'+img+'" alt="masonry"> </li>';

                    // tabContents += '<a class="elem" href="'+img+'" title="image 1" data-lcl-txt="lorem ipsum dolor sit amet" data-lcl-author="someone" data-lcl-thumb="'+img+'"><img src="'+img+'"></img></a>';
