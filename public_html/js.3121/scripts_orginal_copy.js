      ...
      console.log(users);
        if(users != ""){
            ...
            //   $("meta[property='og:url']").attr("content", window.location.href);
                ...
                // tabContents += '</div></div>';
                if(count == 0){
                    getAlbumFiles(valuefolder_name, value.user_id, value.id);
                }
                count++;
            });
        }else{
    ...
    if(userIdVal == userId){
        ...
        $.ajax({
            ...
            success: function(result){
                ...
                tabContents += '<div id="masonryGallery'+albumId+'">';
                ...
                $.each(images, function(key1,value1) {
                    ...
                    if(value1.hide == 1){
                        ...
                        // tabContents += '</div>';
                            ...
                            tabContents += '</ul>';
                            tabContents += '<a class="elem" href="'+thumb_img+'" title="image 1" data-lcl-txt="lorem ipsum dolor sit amet" data-lcl-author="someone" data-lcl-thumb="'+thumb_img+'"><img src="'+thumb_img+'" class="image"></img></a>';
                            tabContents += '</div>';
                ...
                setTimeout(function(){
                    // masonryInitialize();
                    // initValik();
                    // $("#masonryGallery"+albumId).justifiedGallery();
...
function saveComments(){
    ...
    return new swal({
        title: "Are you sure?",
        text: "You want to save this comment",
        icon: false,
        // buttons: true,
        // dangerMode: true,
            ...
            if (confirm.isConfirmed) {
                successFn = function(resp)  {
                    ...
                    if(resp.status == 1){
                        ...
                        swal.fire({
                            icon: 'success',
                            title: "Successfully saved comment",
                            showConfirmButton: false,
                            timer: 1500
                        });
                        // $("#commentUserName").val("");
    ...
    successFn = function(resp)  {
        ...
        if(data != "" && data != null){
            $.each(data, function(key,value) {
                ...
                html += '<i class="bi bi-person-bounding-box" style="font-size: 50px;"></i></div>';
                html += '<cite class="fn"><a>'+value.name+'</a></cite>';
                ...
                html += '<div class="" style="text-align: right;" id="replyCommentbtnDiv_'+value.id+'"><a href="javascript:void(0)" onclick="replyComment('+value.id+');">Reply</a></div>';
    ...
    successFn = function(resp)  {
        ...
        if(data != "" && data != null){
            $.each(data, function(key,value) {
                ...
                html += '<i class="bi bi-person-circle" style="font-size: 40px;"></i></div>';
                html += '<cite class="fn"><a>'+value.created_by+'</a></cite>';
                // html += '<div class="comment-meta"><h6><a>'+dateFormat(value.created_in)+'</a></h6></div>';
                html += '<p style="font-size: 16px; padding-bottom: 0px;">'+value.comment_reply+'</p>';
                // html += '<div class="comment-num">01.</div>';
...
function replyComment(commentId){
    ...
    html += ' <textarea style="height: 100px;" name="commentsReplay_'+commentId+'" id="commentsReplay_'+commentId+'" rows="2" placeholder="Your reply:" class=""></textarea>';
    html += ' </div>';
    html += '<div class="col-md-12">';
